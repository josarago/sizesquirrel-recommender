...
class Trainer:
	...
	def load_data(self):
		...
		logger.info(f"Loaded data: {self.df.shape[1]} columns and {self.df.shape[0]:,} rows")
		
	@staticmethod
	def get_sizing_system(size):
		return "US" if size < 25. else "EURO"
	...
	def get_training_set(self):
		...
		logger.info(f"sorting data by increasing (user-item) `id` number. (chronological order)")		
	...
	def user_sku_mat_parsity(self):
		return self.user_sku_mat_df.notnull().sum().sum() / (self.user_sku_mat_df.shape[0] * self.user_sku_mat_df.shape[1])
